...
BASE_URL = os.environ.get("AGNES_API_BASE", "https://apihub.agnes-ai.com").rstrip("/")
...
def extract_video_id(data: dict[str, Any]) -> str | None:
    ...
    if isinstance(nested, dict):
        return extract_video_id(nested)
    return None


def get_video(video_id: str, *, legacy_task_id: bool = False) -> dict[str, Any]:
    if legacy_task_id:
        return request_json("GET", f"/v1/videos/{urllib.parse.quote(video_id)}")
    query = urllib.parse.urlencode({"video_id": video_id})
    return request_json("GET", f"/agnesapi?{query}")


def poll_video(video_id: str, timeout: int, interval: int, *, legacy_task_id: bool = False) -> dict[str, Any]:
    deadline = time.time() + timeout
    last: dict[str, Any] = {}
    while time.time() < deadline:
        last = get_video(video_id, legacy_task_id=legacy_task_id)
        if last.get("error"):
...
def cmd_video(args: argparse.Namespace) -> None:
    ...
    data = poll_video(video_id, args.timeout, args.interval)
...
def cmd_video_get(args: argparse.Namespace) -> None:
    data = get_video(args.video_id, legacy_task_id=args.legacy_task_id)
    urls = extract_video_urls(data)
    output_result(
        "video-result",
        data,
        urls=urls,
        status=str(data.get("status", "")) if data.get("status") is not None else None,
        next_steps=[] if urls else [f"python scripts/agnes_api.py video-get {args.video_id}"],
        raw_only=args.raw,
    )
    if data.get("error"):
        raise SystemExit(1)
...
def create_video_case(name: str, payload: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    video_id = extract_video_id(created)
    if not video_id:
        raise SystemExit(f"{name}-create response did not include video_id or id: {json.dumps(created)}")
    retrieved = (
        poll_video(video_id, args.video_timeout, args.video_interval)
        if args.poll_video
        else get_video(video_id)
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    video_get = sub.add_parser("video-get", help="Retrieve a video by video_id.")
    video_get.add_argument("video_id")
    video_get.add_argument(
        "--legacy-task-id",
        action="store_true",
        help="Use the older /v1/videos/{task_id} compatibility endpoint.",
    )
