...
get_mcporter_config() {
    mcporter config get kdocs-qclaw --json 2>/dev/null || true
}
...
write_mcporter_config() {
    local token="${1:-}"
    local args=(
        config add kdocs-qclaw "$MCP_URL"
        --header "X-Skill-Version=$SKILL_VERSION"
        --header "X-Request-Source=qclaw"
        --transport http
        --scope home
    ...
    mcporter config remove kdocs-qclaw >/dev/null 2>&1 || true
...
echo ""
echo "🧪 验证配置..."
if mcporter config get kdocs-qclaw >/dev/null 2>&1; then
...
echo ""
echo "📖 使用方法："
echo "   mcporter call kdocs-qclaw.scrape_url"
echo ""
