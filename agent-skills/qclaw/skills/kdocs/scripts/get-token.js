...
function ensureMcporter() {
  ...
  if (!isCommandAvailable("mcporter")) {
    throw new Error(
      "mcporter is required to save the kdocs-qclaw config. Default behavior will not auto-install globally. Install mcporter manually, or rerun with --auto-install-mcporter (or -AutoInstallMcporter)."
    );
  }
}
...
function setMcporterConfig(token, version) {
  ...
  try {
    runCommand("mcporter", ["config", "remove", "kdocs-qclaw"]);
  } catch (error) {
...
async function main() {
  ...
  console.log("");
  console.log("[OK] Login successful. kdocs-qclaw Skill is ready.");

  try {
    setMcporterConfig(token, getSkillVersion());
    removeLegacyEnvTokenKey();
    console.log("[OK] Updated kdocs-qclaw config in mcporter.");
  } catch (error) {
